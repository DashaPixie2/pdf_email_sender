...
app.post('/send-email', upload.fields([{ name: 'idFront' }, { name: 'idBack' }]), async (req, res) => {
    ...
    const addFooter = (pageNumber) => {
        doc.fontSize(10)
            .fillColor('gray')
            .text(`Initials: ${initials}`, 50, 750)
            .text(`Signed by: ${firstName} ${surname}`, 300, 750, { align: 'right' })
            .text(`Page ${pageNumber}/${totalPages}`, 300, 765, { align: 'right' });
    };
    ...
    doc.text(`I am not a hemophiliac (bleeder). I do not have Diabetes, Epilepsy, Hepatitis, Aids or any other communicable disease. I am not under the influence of alcohol and or drugs.
...
I agree to release and forever discharge and hold harmless Dasha Pixie from any and all claims, damages, and legal actions arising from or connected in any way with my tattoo of the procedures and conduct used to apply my Tattoo.`);
    ...
    if (signature) {
        ...
        doc.text(`Signed by: ${firstName} ${surname}`, { continued: true }).text(` on ${currentDate}`, { align: 'left' });
    ...
    // PAGE 2
    doc.fontSize(12).text(`I confirm that the signature provided is my own, created by me personally and electronically.
...
or third party verification will not in any way affect the enforceability of your E-Signature. You may request a paper version of an electronic record by writing to us.`, { align: 'justify' });

    doc.moveDown();
    if (signature) {
        const signaturePath2 = `./uploads/signature_${Date.now()}_2.png`;
        const base64Data2 = signature.replace(/^data:image\/png;base64,/, "");
        fs.writeFileSync(signaturePath2, Buffer.from(base64Data2, 'base64'));

        doc.image(signaturePath2, { fit: [150, 80], align: 'left' });
        doc.text(`Signed by: ${firstName} ${surname}`, { continued: true }).text(` on ${currentDate}`, { align: 'left' });
