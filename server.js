...
app.post('/send-email', upload.fields([{ name: 'idFront' }, { name: 'idBack' }]), async (req, res) => {
    ...
    const addPageWithFooter = (pageNumber, totalPages) => {
        doc.addPage();

        // Header and Footer
        doc.fontSize(8)
            .text(`Initials: ${initials}    Signature: [Signed Electronically]`, 50, doc.page.height - 50, {
                align: 'left'
            });

        doc.text(`${pageNumber}/${totalPages}`, doc.page.width - 100, doc.page.height - 50, {
            align: 'right'
        });
    };
    ...
    and Uniform Electronic Transactions Act (UETA).
    By clicking/tapping/touching/selecting or otherwise interacting with the "Submit" button below,
    you are consenting to signing this Document electronically.
    ...
    You agree that no certification authority or other third party verification is necessary to validate your E-Signature
    and that the lack of such certification or third party verification will not in any way affect the enforceability of your E-Signature.
    You may request a paper version of an electronic record by writing to us.
    `;

    // Страница 1
    addPageWithFooter(1, 2);
    ...
    if (signature) {
        ...
        doc.moveDown();
        doc.text(`Signed by: ${fullName}  Date: ${today}`);
        doc.image(signaturePath, { fit: [150, 80], align: 'center' });
        fs.unlinkSync(signaturePath);
    }

    // Страница 2
    addPageWithFooter(2, 2);
    doc.fontSize(12).text(textBlock2);
    doc.moveDown();
    doc.text(`Signed by: ${fullName}  Date: ${today}`);
    if (signature) {
